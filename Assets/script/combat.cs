...
public class combatant {
	...
	/*
	 * Increases _combatant's level by one.
	 * Randomly increases one of _combatant's attributes.
	 * Fully heals _combatant.
	 * Returns the increased stat.
	 */
	public static int level_up(combatant _combatant) {
		int attribute_increased;

		attribute_increased = Random.Range(STRENGTH, INTELLIGENCE + 1);

		_combatant._attributes[attribute_increased] += 1;
...
public static class combat {
	...
	public static string engagement(
		...
		if (defender._trap) {
			...
			damage = offense._attributes[STRENGTH];

			defense._health -= damage;

			log += defense._name + " takes " + damage;
			log += " from " + offense._name + "!";
		...
		offense._experience += defense._experience;
